...
+ (void)requestWithMethod:(MSRequestMethod)method url:(NSString *)URL  parameters:(NSDictionary *)parameters cachePolicy:(MSCachePolicy)cachePolicy success:(MSHttpSuccess)success  failure:(MSHttpFail)failure{
    ...
    NSString * url = kFetchMyDefault(@"address");
    [MSNetwork setBaseURL:url];
...
//    [MSNetwork openNetworkActivityIndicator:YES];
    //设置公用请求参数
//    NSString * token = kFetchMyDefault(@"token");
//    NSDictionary *dic =@{@"token":token};
    
//  @{@"accountToken":@{@"tokenId":@"",@"userId":@"",@"initDate":@"",@"clientType":@"mobilType",@"tokenKey":@""}};
//    [MSNetwork setBaseParameters:dic];
//    [MSNetwork setHeadr:dic];
    //可以设置全局等待的指示器
