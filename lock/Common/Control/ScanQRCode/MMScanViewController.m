...
- (void)viewDidLoad {
    ...
    [self gennerateNavigationItemReturnBtn:@selector(returnCick)];
...
- (void)initScanType{
    if (self.scanType == MMScanTypeAll) {
        ...
        _scanRect = CGRectFromString([self scanRectWithScale:1][0]);
        self.output.rectOfInterest = _scanRect;
        _tipTitle.text = STR_QR_CODE_TIPS;
...
- (void)drawBottomItems
{
    ...
    [_scanTypeQrBtn setTitle:STR_QR_CODE forState:UIControlStateNormal];
    ...
    [_scanTypeBarBtn setTitle:STR_BAR_CODE forState:UIControlStateNormal];
...
- (void)setNavItem:(MMScanType)type {
    if(type == MMScanTypeBarCode) {
        ...
        UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithTitle:STR_PHOTO style:UIBarButtonItemStylePlain target:self action:@selector(openPhoto)];
...
- (void)changeScanCodeType:(MMScanType)type {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        _tipTitle.text = type == MMScanTypeQrCode ? STR_QR_CODE_TIPS : STR_BAR_CODE_TIPS;
...
- (void)openPhoto
{
    ...
    {
        NSString *tipMessage = [NSString stringWithFormat:STR_PHOTO_SET,appName];
        [self showError:tipMessage andTitle:STR_PHOTO_NO_POWER];
    }
}
...
- (BOOL)isAvailableCamera {
    if([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])
    {
        ...
        if (authorizationStatus == AVAuthorizationStatusRestricted ||
            ...
            NSString *tipMessage = [NSString stringWithFormat:STR_CAMERA_SET,appName];
            [self showError:tipMessage andTitle:STR_CAMERA_NO_POWER];
...
- (void)showError:(NSString*)str {
    [self showError:str andTitle:STR_TIPS];
}
...
- (void)showError:(NSString*)str andTitle:(NSString *)title
{
    ...
    UIAlertAction *action1 = ({
        UIAlertAction *action = [UIAlertAction actionWithTitle:STR_KNOW style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)recognizeQrCodeImage:(UIImage *)image onFinish:(void (^)(NSString *result))finish {
    ...
    if ([[[UIDevice currentDevice]systemVersion]floatValue] < 8.0 ) {
        ...
        [self showError:STR_SUPPORT_EIGHT_SYSTEM];
        return;
    }
