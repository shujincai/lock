...
- (void)requestSetBlackListKeyResultInfo:(ResultInfo *)info{
    ...
    if (info.feedBackState == NO) {
        ...
        RequestBean * request = [[RequestBean alloc]init];
        [MSHTTPRequest PUT:[NSString stringWithFormat:kSetKeyFlag,self.keyInfo.key_id,2] parameters:[request toDictionary] cachePolicy:MSCachePolicyOnlyNetNoCache success:^(id  _Nonnull responseObject) {
            [MBProgressHUD hideHUD];
            NSError * error = nil;
