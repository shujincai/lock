...
@interface WriteListCreateVC ()<UITableViewDataSource,UITableViewDelegate,SetKeyControllerDelegate,KeyDelegate>
...
- (void)requestSetBlackListKeyResultInfo:(ResultInfo *)info{
    if (info.feedBackState == NO) {
        ...
        [self setWhiteList:self.keyInfoB.key_id];
    }
}
...
- (void)onSetBlockListKey:(Result *)result {
    if (result.ret == NO) {
        ...
        [self setWhiteList:self.keyInfoC.keyId];
    }
}
