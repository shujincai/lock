//
//  SwitchLockDateViewController.m
//  lock
//
...
@interface SwitchLockDateViewController ()<UITableViewDataSource,UITableViewDelegate,SetKeyControllerDelegate,KeyDelegate>
...
@implementation SwitchLockDateViewController
...
-(void)onSetDateTime:(Result*)result {
    if (result.ret == NO) {
        ...
        if (_keyInfoC.device == 1284) { //指纹蓝牙钥匙
            ...
            //锁号
            NSMutableArray<LockInfo4*> * lockIds = [[NSMutableArray<LockInfo4*>  alloc]init];
            //时间块，时间片
            NSMutableArray<DateSection*> * dates = [[NSMutableArray<DateSection*>  alloc]init];
            for (int i = 0; i < _taskBean.unlocklist.count; i++) {
                //多时间段
                NSMutableArray<TimeSection*> * times = [[NSMutableArray<TimeSection*>  alloc]init];
                
                UnlockDateListBean * unlockBean = [_taskBean.unlocklist objectAtIndex:i];
                for (LockDateListBean * lockBean in unlockBean.locklist) {
                    [lockIds addObject:[[LockInfo4 alloc]initLockInfo4:lockBean.lockno timeId:i+1]];
                }
                for (MyTaskTimeRangeListBean * timeList in unlockBean.timelist) {
                ...
                NSDate * beginDate = [formatter dateFromString:unlockBean.begindate];
                NSDate * endDate = [formatter dateFromString:unlockBean.enddate];
                [dates addObject:[[DateSection alloc] initSection:beginDate to:endDate times:times]];
            }
            UserKeyInfo4 *userkeyInfo = [[UserKeyInfo4 alloc] initUserKeyInfo4:dates lockIds:lockIds];
            [self.bleKeysdk setUserKey4:userkeyInfo isOnline:YES];
        }
    }
}
//指纹授权
